...
const EditTodoForm = () => {
	...
	const { saveEditTodo, activeCollection, editToDoItem, handleEditCancel } =
		useMyContext();
	...
	const handleToSave = () => {
		if (date && task && priority && editToDoItem !== undefined) {
			...
			saveEditTodo(todoItem);
		}
	};
	// const d = new Date(editToDoItem?.date);
	...
	const form = useForm<z.infer<typeof FormSchema>>({
		...
		defaultValues: {
			todo: editToDoItem === undefined ? '' : editToDoItem.task,
			date:
				editToDoItem === undefined ? new Date() : new Date(editToDoItem.date),
			priority: String(editToDoItem === undefined ? '' : editToDoItem.priority),
		},
	});
